...
//		}
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>index.php/site_user/analisa/simpan",
			data	: string,
			cache	: false,
...
//		}
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>index.php/site_user/analisa/ajukan",
			data	: string,
			cache	: false,
			success	: function(data){
				alert(data);
	...
	$("#hapus").click(function(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>index.php/site_user/analisa/hapus",
			data	: string,
			cache	: false,
			success	: function(data){
				alert(data);
...
function editData(ID){
	...
	$.ajax({
		type	: "POST",
		url		: "<?php echo site_url(); ?>index.php/site_user/analisa/cari",
		data	: "cari="+cari,
		dataType: "json",
		success	: function(data){
			//alert(data.ref);
...
//		$data = $this->model_data->data_analisa($username);
		$data = $this->model_data->data_analisa("user",$username);			
		$i=1;
            ...
            <td class="td-actions"><center>
            	<div class="hidden-phone visible-desktop action-buttons">
                 <?php if($dt->status_data==2 or $dt->status_data==8){ 
                    ...
                    <a class="green" href="#modal-table" onclick="javascript:editData('<?php echo $dt->id_analisa;?>')" data-toggle="modal">
    ...
    <div class="modal-body no-padding">
        <div class="row-fluid">
            <form class="form-horizontal" name="my-form" id="my-form">
				...
				<div class="control-group">
                    <label class="control-label" for="form-field-1">Jenis Kredit</label>
                    <div class="controls">
                        <select name="jenis_kredit" id="jenis_kredit" class="span6" >
                            <option value="" selected="selected">-Pilih-</option>
								<?php 
									$kredit = array('Kredit Angsuran Lainnya' , 'KUR Mikro' , 'KUR Kecil' , 'KPP SS' , 'KPR ST FLPP' , 'Kredit SIPP' , 'Kredit KUSS' , 'Kredit KISS' , 'Kredit Mikro Sumut Bermatabat' , 'Kredit Multi Guna (KMG)' , 'Kredit Pensiun' , 'Kredit Pra Pensiun' , 'Kredit Pensiunan' , 'Kredit Karyawan');
								
									$no = 1;
								foreach($kredit as $nmkredit){
								?>
									<option value="<?=$nmkredit?>"><?=$nmkredit?></option>
								<?php $no++;}?>
                            </select>
                    </div>
                </div>
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <select name="mampu_memenuhi_tagihan" id="mampu_memenuhi_tagihan" class="span5" >
                 ...
                 <div class="control-group">
                    ...
                    <div class="controls">
                        <select name="cukup_agunan" id="cukup_agunan" class="span5" >
                        	<option value="1" selected="selected">Ya</option>
                            <option value="2">Tidak</option>
                            <option value="3">Ya Dengan Catatan</option>
                        </select>
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <select name="pembiayaan_wajar" id="pembiayaan_wajar" class="span5">
                        	<option value="1" selected="selected">Ya</option>
                            <option value="2">Tidak</option>
                            <option value="3">Ya Dengan Catatan</option>
                        </select>
