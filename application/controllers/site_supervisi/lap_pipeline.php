...
//	 * TIM : Edy Nasri, Aldi Novialdi Rusdiansyah, Eka Juliananta
	...
	public function index()
	{
		...
		if(!empty($cek) && $level=='supervisi'){
			...
			$d['content']= 'site_supervisi/lap_pipeline/form';
	...
	public function cari_data()
	{
		...
		if(!empty($cek) && $level=='supervisi'){
			...
			$status_kondisi_data = $this->input->post('status_kondisi_data');
						...
						$dt['data'] = $q;
						echo $this->load->view('site_supervisi/lap_pipeline/view',$dt);		
...
//			$jlh_prospek = $this->model_data->sum_data_nominal_tabel($level,$username);
...
//			$sess_data['nama_user'] = $this->model_data->ambil_data_user("nama_user",$username);
			$sess_data['nama_user'] = $this->model_data->ambil_data_user("nama_user","supervisi",$username);			
...
//			$sess_data['rp_wajib_expansi'] = $this->model_data->ambil_data_rencana_kerja("rp_wajib_expansi",$username,date("Y"));
//			$sess_data['rp_target_os'] = $this->model_data->ambil_data_rencana_kerja("rp_target_os",$username,date("Y"));
//			$sess_data['rp_potensi_on_book'] = $jlh_jlh_on_book;
//			$sess_data['rp_lebih_kurang'] = $this->model_data->ambil_data_rencana_kerja("rp_lebih_kurang",$username,date("Y"));
...
//			if($r>0){
				...
				//foreach($data->result() as $t){
					...
					//$w = array(10,25,30,30,80,10,30,35,25);
						...
						$pdf->Cell(190,4,'Nama Supervisi : '.$nama_user ,0,1,'C');
						$pdf->Ln(5);	
...
//				redirect('site_supervisi/lap_pipeline');
				//echo "Maaf Tidak ada data";
//			}
		}else{
			redirect('login','refresh');
...
//			if($r>0){
            ...
            <p>Nama Supervisi : <?php echo $nama_user; ?> <p>
            <p></p>
