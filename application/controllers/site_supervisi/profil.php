...
class Profil extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek) && $level=='user'){
			...
			$d['id_user'] = $this->session->userdata('username');
			$d['nama_lengkap'] = $this->session->userdata('nama_user');
			$d['foto']= $this->model_data->cari_foto_user($this->session->userdata('username'));
...
//				$data['prodi'] = $dt->prodi;
	...
	public function simpan_profil()
	{
		...
		if(!empty($cek) && $level=='user'){
			...
			$dt['id_user_supervisi1'] = $this->input->post('id_user_supervisi1');
			$dt['id_user_supervisi2'] = $this->input->post('id_user_supervisi2');
//			$dt['tanggal_lahir'] = $tgl_lhr;
			$dt['status_user'] = $this->input->post('status_user');
